...
DTK_AKID=`grep AccessKeyId $TOKENFILE | cut -f2 -d ':' | sed s/\"//g | tr -d ',' | xargs --`
DTK_SAK=`grep SecretAccessKey $TOKENFILE | cut -f2 -d ':' | sed s/\"//g | tr -d ',' | xargs --`
DTK_ST=`grep SessionToken $TOKENFILE | cut -f2 -d ':' | sed s/\"//g | tr -d ',' | xargs --`
echo -e "\n " >> $AWSOUT/credentials
...
AFT_AKID=`grep AccessKeyId $TOKENFILE | cut -f2 -d ':' | sed s/\"//g | tr -d ',' | xargs --`
AFT_SAK=`grep SecretAccessKey $TOKENFILE | cut -f2 -d ':' | sed s/\"//g | tr -d ',' | xargs --`
AFT_ST=`grep SessionToken $TOKENFILE | cut -f2 -d ':' | sed s/\"//g | tr -d ',' | xargs --`
echo -e "\n " >> $AWSOUT/credentials
